...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        // В транзакции удаляем сначала логи, потом кошку
        await prisma.$transaction([
            prisma.auditLog.deleteMany({ where: { catId: params.id } }),
            prisma.cat.delete({ where: { id: params.id } }),
        ]);

        return NextResponse.json({ message: 'Cat deleted successfully' }, { status: 200 });
    } catch (error) {
...
export async function PATCH(request: Request, { params }: { params: { id:string } }) {
    ...
    try {
        ...
        let changes: string[] = [];
        if (body.name && body.name !== currentCat.name) {
            changes.push(`имя с '${currentCat.name}' на '${body.name}'`);
        }
        if (body.birthYear && body.birthYear !== currentCat.birthYear) {
            changes.push(`год рождения с '${currentCat.birthYear || '?'}' на '${body.birthYear}'`);
        }
        ...
        // Добавляем проверку для заметок
